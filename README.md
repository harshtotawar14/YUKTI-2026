...
- `docs/final-research-grounded-frontend-master-prompt.md` — **LATEST / PRIMARY frontend execution prompt**.
- `docs/voice-request-worker-delivery-addendum.md` — **MANDATORY voice-flow addendum**. Use it with the primary frontend prompt.
...
- deliver Customer Voice → transcript → structured confirmed fields → booking → eligible-worker offer → worker preferred-language text + Listen → Accept/Reject;
- ensure worker-scoped voice notifications go only to the matched/offered worker;
- preserve the same voice request context when the first worker rejects or times out and the next eligible worker is offered;
