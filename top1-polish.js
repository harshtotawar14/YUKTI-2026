(() => {
  ...
  async function refreshReadiness(force=false){
    ...
    let box=document.getElementById('judgeReadiness');if(!box){box=document.createElement('div');box.id='judgeReadiness';box.className='judge-note judge-readiness-box';hero.appendChild(box);}box.textContent='Checking Golden Demo readiness…';
  ...
  function start(){
    ...
    document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;const connectedCancel=document.querySelector('#connectedModalRoot [data-modal-cancel]');if(connectedCancel){connectedCancel.click();return;}if(document.getElementById('judgeResetModal'))closeJudgeResetModal();});
    document.addEventListener('click',event=>{
      const reset=event.target.closest?.('#judgeReset');
      if(reset){event.preventDefault();event.stopImmediatePropagation();showJudgeResetModal();return;}
