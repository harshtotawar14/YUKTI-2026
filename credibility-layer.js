(() => {
  ...
  const RESEARCH=[
    ...
    {title:'Google Maps Routes API Overview',year:'Current documentation',url:'https://developers.google.com/maps/documentation/routes/overview',insight:'Distance, routes and ETA should come from geospatial routing services.',decision:'20 KM configured policy today; production route/ETA remains integration-ready.'},
  ...
  function originBadge(){return '<span class="cred-origin">● BACKEND-DERIVED DATA</span>';}
  ...
  function validationCard(){
    return `<div class="cred-validation">
      <div><span class="cred-status planned">FIELD-INFORMED</span><b>Stakeholder validation evidence</b><p>Field interaction in Kolhapur informed product decisions through recorded operational findings, signed/stamped acknowledgement and reviewed documentation. This evidence is research input, not government endorsement or a completed impact pilot.</p></div>
      <details><summary>What the field work validated</summary><div class="cred-mini-grid"><span>Worker discovery</span><span>Service operations</span><span>Digital records</span><span>Welfare & safety</span></div><p>Findings were translated into product decisions; measurable impact remains a pilot KPI, not a claimed outcome.</p></details>
    </div>`;
  }
  ...
  function renderFallback(data){
    ...
    if(!f?.available)return `<div class="cred-unavailable">${esc(f?.message||'Run the connected workflow to create fallback evidence.')}</div>`;
  ...
  function renderWorkerGrowth(data){
    ...
    return `${originBadge()}<div class="cred-live-grid"><span><b>${esc(w.worker)}</b><small>${esc(w.verificationStatus)}</small></span><span><b>${Number(w.completedJobs||0)}</b><small>Recorded completed jobs</small></span><span><b>₹${Number(w.recordedDemoEarnings||0).toFixed(0)}</b><small>Recorded earnings</small></span><span><b>${Number(w.trainingRecommendations||0)}</b><small>Training recommendations</small></span></div><div class="cred-worker-roadmap"><span>Verified work history <b>Implemented</b></span><span>Training recommendation <b>Human-reviewed</b></span><span>Certificate expiry automation <b>${esc(w.certificateExpiryAutomation)}</b></span><span>Insurance / welfare APIs <b>${esc(w.insuranceIntegration)}</b></span></div>`;
  }
  ...
  function renderCommand(data){
    ...
    return `${originBadge()}<div class="cred-live-grid six"><span><b>${c.registeredWorkers}</b><small>Registered workers</small></span><span><b>${c.verifiedWorkers}</b><small>Verified</small></span><span><b>${c.availableWorkers}</b><small>Available</small></span><span><b>${c.pendingVerification}</b><small>Pending verification</small></span><span><b>${c.openDemoComplaints}</b><small>Open complaints</small></span><span><b>${c.openCapacityRequests}</b><small>Open capacity requests</small></span></div>`;
  }
  ...
  function credibilityHtml(){
    return `<div class="judge-card cred-hero"><span class="judge-badge">SYSTEM CREDIBILITY & EVIDENCE</span><h2>Evidence behind SanPaid’s design and implementation</h2><p>SanPaid separates backend-derived proof, explanatory scenarios, sandbox flows, planned validation and future integrations instead of blending them together.</p><div class="cred-legend"><span class="live">Implemented / backend-derived</span><span class="connected">Connected workflow</span><span class="sandbox">Sandbox / simulation</span><span class="future">Future integration</span><span class="risk">Excluded / needs validation</span></div></div>
    <div class="judge-grid two" style="margin-top:12px">
      <div class="judge-card">${validationCard()}<div class="cred-team-note"><b>Evidence boundary:</b> field research supports problem and design validation; numeric impact remains unclaimed until a measured pilot.</div></div>
    ...
    <div class="judge-card" style="margin-top:12px"><span class="judge-badge">DEVICE & BROWSER QA</span><h3>Release verification checklist</h3><p>Required widths: 360 · 375 · 390 · 412 · 430 · 768 · 1024 · 1440. Browsers: Chrome desktop/Android, Edge desktop and Safari/iPhone if available.</p><span class="cred-status planned">RELEASE CHECK</span><div class="judge-note">Do not mark PASS until a real device/browser test is performed and recorded with device, browser, date, tester and result.</div></div>`;
  }
