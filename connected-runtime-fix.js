(() => {
  ...
  class ConnectedPollingSource{
    ...
    async tick(){if(this.closed)return;if(document.hidden||!connectedShellActive()){this.schedule(1800);return;}if(!getToken()){this.readyState=0;this.schedule(1800);return;}try{const response=await window.fetch('/api/connected/snapshot',{method:'GET',cache:'no-store'});if(!response.ok)throw new Error(`snapshot_${response.status}`);const snapshot=await response.json();const sig=this.signature(snapshot);this.readyState=1;if(!this.opened){this.opened=true;if(typeof this.onopen==='function'){try{this.onopen({type:'open'})}catch{}}}if(sig!==this.lastSignature){this.lastSignature=sig;this.emit('snapshot',snapshot);}setTop('● Live','#8ee2b5');this.schedule(3000);}catch(error){this.readyState=0;setTop('● Reconnecting…','#ffb66e');if(typeof this.onerror==='function'){try{this.onerror(error)}catch{}}this.schedule(4500);}}
  ...
  window.SanPaidConnectedTransport={backend:BACKEND,mode:'DIRECT_RENDER_BEARER_POLLING_OPTIMIZED',hasSession:()=>!!getToken(),clearSession:()=>setToken('')};
  ...
  function start(){checkHealth();window.addEventListener('online',checkHealth);window.addEventListener('offline',()=>syncLandingTruth(false));document.addEventListener('visibilitychange',()=>{if(!document.hidden)checkHealth();});setInterval(checkHealth,45000);}
