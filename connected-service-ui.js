(() => {
  ...
  function friendly(e){const s=Number(e?.status||0);if(s===401)return 'Your demo session expired. Please log in again.';if(s===409)return 'This step was already completed on another device.';if(s>=500)return 'Service temporarily unavailable. Please retry.';return e?.message||'This step could not be completed. Please retry.';}
  ...
  async function renderWorkerLifecycle(){
    ...
    try{
      ...
      const id=Number(accepted.bookingId);const status=String(accepted.status||'');const storedToken=localStorage.getItem(`sanpaid_service_start_token_${id}`)||'';
      let action='';
  ...
  async function runWorkerAction(action,id,button){if(busy)return;busy=true;const old=button?.textContent;if(button){button.disabled=true;button.textContent=action==='travel'?'Starting travel…':action==='arrive'?'Updating arrival…':action==='identity'?'Verifying…':action==='start'?'Starting service…':'Requesting completion…';}try{let result;if(action==='travel')result=await post(`/api/connected/jobs/${id}/travel`);if(action==='arrive')result=await post(`/api/connected/jobs/${id}/arrive`);if(action==='identity'){result=await post(`/api/connected/jobs/${id}/identity`);if(result.token)localStorage.setItem(`sanpaid_service_start_token_${id}`,result.token);}if(action==='start')result=await post(`/api/connected/jobs/${id}/start`);if(action==='complete-request')result=await post(`/api/connected/jobs/${id}/completion-request`);message(action==='identity'?'Identity verified in sandbox. A one-time service verification code is ready.':'Service status updated across devices.');lastWorkerSignature='';}catch(e){message(friendly(e),'error');if(button){button.disabled=false;button.textContent=old;}}finally{busy=false;setTimeout(refresh,180);}}

  async function renderCustomerVerification(){const host=ensureHost('connectedCustomerVerificationHost','.connected-grid');if(!host)return;const bookingId=Number(localStorage.getItem('sanpaid_connected_booking_id')||0);if(!bookingId){if(lastCustomerSignature!=='EMPTY'){host.innerHTML='';lastCustomerSignature='EMPTY';}return;}let booking=null;try{booking=await req(`/api/connected/customer/bookings/${bookingId}`);}catch{}if(!booking){if(lastCustomerSignature!=='EMPTY'){host.innerHTML='';lastCustomerSignature='EMPTY';}return;}const status=String(booking.status||'');const completion=status==='AWAITING_CUSTOMER_CONFIRMATION'?`<button class="btn primary" id="connectedConfirmCompletion">Confirm Service Completed</button>`:'';const saved=localStorage.getItem(`sanpaid_customer_start_token_${bookingId}`)||'';const signature=JSON.stringify([bookingId,status,saved]);const html=`<div class="connected-card"><span class="connected-step-label">TRUST CHECK</span><h3>Confirm Your Booked Worker</h3><p>After the worker arrives and passes the sandbox identity check, enter the one-time service verification code shown on the worker device.</p><div class="connected-status"><span class="badge b-green">${esc(human(status))}</span><b>${esc(booking.bookingCode||'')}</b></div><div class="connected-form"><div class="field"><label>One-Time Service Verification Code</label><input id="connectedStartToken" autocomplete="off" placeholder="Paste code from Worker device" value="${esc(saved)}"></div><div class="connected-actions"><button class="btn secondary" id="connectedVerifyToken">Check Worker</button><button class="btn primary" id="connectedConfirmWorker" disabled>Confirm Booked Worker</button>${completion}</div><div id="connectedWorkerIdentityPreview"></div><div id="connectedCustomerVerificationMessage"></div></div></div>`;if(setMarkup(host,signature,html,'customer')){const input=host.querySelector('#connectedStartToken');host.querySelector('#connectedVerifyToken').onclick=()=>verifyToken(input.value.trim(),bookingId);host.querySelector('#connectedConfirmWorker').onclick=()=>confirmToken(input.value.trim(),bookingId);host.querySelector('#connectedConfirmCompletion')?.addEventListener('click',e=>confirmCompletion(bookingId,e.currentTarget));if(saved)verifyToken(saved,bookingId,false);}}

  async function verifyToken(token,bookingId,showErrors=true){const preview=document.getElementById('connectedWorkerIdentityPreview');const confirm=document.getElementById('connectedConfirmWorker');if(!token){if(showErrors)message('Enter the one-time verification code from the worker device.','warn');return;}try{const d=await req(`/api/connected/service-start/${encodeURIComponent(token)}`);if(Number(d.bookingId)!==Number(bookingId))throw Object.assign(new Error('This code belongs to another booking.'),{status:409});localStorage.setItem(`sanpaid_customer_start_token_${bookingId}`,token);preview.innerHTML=`<div class="connected-success" style="margin-top:10px"><b>Worker Identity Verified ✓</b><br>${esc(d.workerName)} · ${esc(d.workerVerification)}<br>${esc(d.cooperative||'Cooperative')}<br>${esc(d.service)} · ${esc(d.bookingCode)}</div>`;confirm.disabled=false;}catch(e){confirm.disabled=true;preview.innerHTML=`<div class="connected-error" style="margin-top:10px">${esc(friendly(e))}</div>`;}}
  async function confirmToken(token,bookingId){if(!token)return message('Check the worker verification code first.','warn');const button=document.getElementById('connectedConfirmWorker');const old=button?.textContent;if(button){button.disabled=true;button.textContent='Confirming worker…';}try{await post(`/api/connected/service-start/${encodeURIComponent(token)}/confirm`);localStorage.removeItem(`sanpaid_customer_start_token_${bookingId}`);message('Booked worker confirmed. Start Service is now unlocked on the worker device.');lastCustomerSignature='';}catch(e){message(friendly(e),'error');if(button){button.disabled=false;button.textContent=old;}}setTimeout(refresh,200);}
  async function confirmCompletion(bookingId,button){const old=button?.textContent;if(button){button.disabled=true;button.textContent='Confirming completion…';}try{await post(`/api/connected/customer/bookings/${bookingId}/complete`);message('Service completion confirmed.');lastCustomerSignature='';}catch(e){message(friendly(e),'error');if(button){button.disabled=false;button.textContent=old;}}setTimeout(refresh,200);}

  function schedule(delay=4600){clearTimeout(timer);timer=setTimeout(async()=>{await refresh();schedule(document.hidden?8000:4600);},delay);}
  function start(){setTimeout(refresh,350);schedule();document.addEventListener('click',event=>{if(event.target.closest('[data-open-connected],#connectedDemoBtn,[data-connected-persona],#connectedLogin'))setTimeout(refresh,420);},true);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh();});}
