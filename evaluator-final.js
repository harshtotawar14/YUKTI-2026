(() => {
  ...
  const MATCH_CANDIDATES = [
    {id:'rahul',name:'Rahul',distance:1.8,skill:'Electrician',verified:false,available:true,workload:'Low',reason:'Verification incomplete'},
    {id:'amit',name:'Amit',distance:2.3,skill:'Electrician',verified:true,available:true,workload:'Lower recent workload',reason:''},
    {id:'suresh',name:'Suresh',distance:3.1,skill:'Electrician',verified:true,available:false,workload:'Balanced',reason:'Not currently available'},
    {id:'priya',name:'Priya',distance:4.0,skill:'Electrician',verified:true,available:true,workload:'Balanced recent workload',reason:''},
    {id:'sanjay',name:'Sanjay',distance:6.2,skill:'Plumber',verified:true,available:true,workload:'Low',reason:'Required skill missing'}
  ];
  ...
  function rankEligible(eligible){
    // Deterministic, explainable ordering for the evaluator-facing prototype.
    // Eligibility is a hard gate. Ranking never makes an ineligible worker eligible.
    return eligible.slice().sort((a,b)=>{
      const workloadRank = w => /lower/i.test(w.workload) ? 0 : 1;
      return workloadRank(a)-workloadRank(b) || a.distance-b.distance || a.name.localeCompare(b.name);
    });
  }
  ...
  function renderRanking(){
    ...
    if(!root)return;
    if(!demoState.ranked.length){
      root.innerHTML='<div class="eval-candidate"><div><strong>No eligible worker</strong><small>No verified worker currently meets all eligibility conditions.</small></div><span class="eval-status pending">REVIEW</span></div>';
      return;
    }
  ...
  function renderOffer(){
    const root=$('#evalOfferRoot');
    if(!root)return;
    if(demoState.offeredIndex<0 || !demoState.ranked[demoState.offeredIndex]){
      root.innerHTML='<div class="eval-offer-card"><h4>Opportunity not offered yet</h4><p>Run the Eligibility Gate and Fair & Explainable Ranking first.</p></div>';
      return;
    }
    const worker=demoState.ranked[demoState.offeredIndex];
    root.innerHTML=`<div class="eval-offer-card">
      <h4>OPPORTUNITY RECEIVED — ${esc(worker.name)}</h4>
      ...
      <div class="eval-offer-actions">
        <button class="btn primary" type="button" data-eval-accept>ACCEPT</button>
        <button class="btn secondary" type="button" data-eval-decline>DECLINE</button>
      </div>
    </div>`;
  }

  function renderAudit(){
    const root=$('#evalAudit');
    if(!root)return;
  ...
  function startMatchingDemo(){
    ...
    demoState.ranked=rankEligible(eligible);
    renderRanking();
    addAudit('Fair & Explainable Ranking Generated');
    if(demoState.ranked.length){
      demoState.offeredIndex=0;
      addAudit(`Opportunity Offered — ${demoState.ranked[0].name}`);
    }
    renderOffer();
    const state=$('#evalMatchState');
    if(state)state.textContent=demoState.ranked.length?'Eligibility passed → ranking complete → worker choice active':'No eligible worker — change schedule/service area or notify Cooperative Admin';
  }

  function declineOffer(){
    const current=demoState.ranked[demoState.offeredIndex];
    if(!current)return;
    addAudit(`${current.name} Declined Opportunity — worker choice respected`);
    const message=$('#evalWorkerMessage');
    if(message){message.hidden=false;message.textContent='Worker choice respected. Offering opportunity to next eligible worker…';}
    demoState.offeredIndex += 1;
    if(demoState.ranked[demoState.offeredIndex]){
      addAudit(`Opportunity Offered — ${demoState.ranked[demoState.offeredIndex].name}`);
      setTimeout(renderOffer,260);
    }else{
      addAudit('Eligible list exhausted — Cooperative Admin review required');
      const root=$('#evalOfferRoot');
      if(root)root.innerHTML='<div class="eval-offer-card"><h4>No eligible worker accepted</h4><p>Change schedule, expand the configured service area under cooperative policy, or notify Cooperative Admin.</p></div>';
    }
  }
  ...
  function acceptOffer(){
    const current=demoState.ranked[demoState.offeredIndex];
    if(!current)return;
    addAudit(`${current.name} Accepted Opportunity`);
    addAudit('Booking Ready for Service-Start Verification');
    const root=$('#evalOfferRoot');
    if(root)root.innerHTML=`<div class="eval-offer-card"><h4>OPPORTUNITY ACCEPTED — ${esc(current.name)}</h4><p>The same service request now continues to Service-Start Verification. No forced assignment was used.</p><div class="eval-worker-message">Next gate: worker identity/booking check + customer confirmation before service starts.</div></div>`;
  }
  ...
  function resetMatching(){
    ...
    demoState.offeredIndex=-1;
    demoState.audit=[];
    const radius=$('#evalRadius');if(radius)radius.value='20';
    renderCandidates();
    renderRanking();
    renderOffer();
    renderAudit();
    const state=$('#evalMatchState');if(state)state.textContent='Ready — current demo configuration: 20 km';
    const msg=$('#evalWorkerMessage');if(msg){msg.hidden=true;msg.textContent='';}
  }

  function wireCapacity(){
    const status=$('#evalCapacityStatus');
    const action=$('#evalCapacityAction');
    if(!action)return;
    let step=0;
    const messages=[
      'Capacity Exchange Suggested — no worker is moved automatically.',
      'Authorized Cooperative / Federation Approval recorded for this demo scenario.',
      'Worker acceptance required — consent remains explicit.',
      'Cross-Cooperative Assignment Confirmed — accountability fields remain visible.'
    ];
    ...
    action.addEventListener('click',()=>{
      ...
      step += 1;
      action.textContent=labels[step] || labels[4];
    });
  }
  ...
  function wire(){
    $('#heroLivePrototype')?.addEventListener('click',()=>openConnected());
    ...
    $('#evalRunMatch')?.addEventListener('click',startMatchingDemo);
    ...
    $('#evalRadius')?.addEventListener('change',event=>{demoState.radius=Number(event.target.value||20);resetMatching();demoState.radius=Number(event.target.value||20);event.target.value=String(demoState.radius);renderCandidates();const state=$('#evalMatchState');if(state)state.textContent=`Ready — configured demo service radius: ${demoState.radius} km`;});

    document.addEventListener('click',event=>{
      const why=event.target.closest('[data-eval-why]');
      if(why){why.closest('.eval-candidate')?.classList.toggle('open');return;}
      const rankWhy=event.target.closest('[data-eval-rank-why]');
      if(rankWhy){const reason=document.querySelector(`[data-eval-rank-reason="${CSS.escape(rankWhy.dataset.evalRankWhy)}"]`);if(reason)reason.style.display=reason.style.display==='block'?'none':'block';return;}
