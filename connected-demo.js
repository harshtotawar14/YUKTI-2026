(() => {
  ...
  let currentUser=null,currentPersona=null,stream=null,activeBookingId=null;
  ...
  async function resolveUser(){
    ...
    try{return (await request('/api/auth/me')).user||null;}catch{return null;}
  }
  ...
  async function loadServiceCatalog(){
    ...
    try{
      const data=await request('/api/connected/customer/services'),rows=data.services||[],prefill=sessionGet('sanpaid_prefill_service_v1'),current=prefill||select.value;
      select.innerHTML=rows.length?rows.map(s=>`<option value="${esc(s.name)}">${esc(s.name)} · from ₹${Number(s.basePrice||0).toLocaleString('en-IN')}</option>`).join(''):'<option value="" disabled>No active services configured</option>';
      if(rows.some(s=>s.name===current))select.value=current;
      if(prefill)sessionSet('sanpaid_prefill_service_v1','');
      window.dispatchEvent(new CustomEvent('sanpaid:services-loaded',{detail:{services:rows}}));
  ...
  function startCustomerVoice(){
    ...
    recognition.onresult=e=>{const transcript=String(e.results?.[0]?.[0]?.transcript||'').trim();if(transcript){document.getElementById('cdProblem').value=transcript;voiceMeta={source:'VOICE',language:lang,transcript};status.textContent='Voice captured. Review the text before sending.';}};
  ...
  async function createConnectedBooking(e){
    e.preventDefault();const error=document.getElementById('cdBookingError'),progress=document.getElementById('cdBookingProgress'),btn=document.getElementById('cdSubmit');error.innerHTML='';
    ...
    const isVoice=voiceMeta.source==='VOICE'&&voiceMeta.transcript===problem;btn.disabled=true;btn.textContent='Finding verified workers…';progress.innerHTML='<div class="connected-progress-note"><b>Checking worker eligibility</b><span>Identity · Skill · Availability · Credentials · Schedule · Service area</span></div>';
    try{
      ...
      activeBookingId=Number(booking.id);sessionSet(BOOKING_KEY,activeBookingId);progress.innerHTML='<div class="connected-success">Request created. Eligible workers are being offered this booking in order.</div>';renderCustomerState(booking,true);await refreshCustomerBooking();signalSync('customer-booking');
  ...
  async function refreshCustomerBooking(){
    ...
    try{const b=await request(`/api/connected/customer/bookings/${activeBookingId}`);renderCustomerState(b);}
  ...
  function stepper(status){const current=bookingStep(status),labels=['Request','Worker Match','Accepted','Arrival','Verification','Service','Payment'];return `<div class="connected-stepper">${labels.map((label,i)=>`<div class="connected-step ${i<current?'done':i===current?'active':''}"><span>${i<current?'✓':i+1}</span><small>${label}</small></div>`).join('')}</div>`;}
  function renderCustomerState(b,force=false){
    const root=document.getElementById('connectedCustomerState');if(!root)return;const signature=JSON.stringify([b.id,b.status,b.workerName,b.workerVerification,b.distance,b.cooperative,b.scheduledAt,b.emergency,b.updatedAt]);if(!force&&signature===lastCustomerSignature)return;lastCustomerSignature=signature;
    const status=String(b.status||''),replacement=status==='FINDING_REPLACEMENT',assigned=!!b.workerName;
    root.innerHTML=`${stepper(status)}<div class="connected-success connected-request-summary"><div><small>Booking ID</small><b>${esc(b.bookingCode||`#${b.id}`)}</b></div><span class="badge ${replacement?'b-orange':'b-green'}">${esc(statusLabel(status))}</span></div><div class="connected-state-line"><b>${replacement?'Finding another verified worker…':'Current status'}</b><p>${esc(statusLabel(status))}</p></div><div class="connected-state-line"><div class="connected-summary-grid"><div><small>Service</small><b>${esc(b.service||'Service')}</b></div><div><small>Scheduled</small><b>${b.scheduledAt?esc(new Date(b.scheduledAt).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'})):'—'}</b></div><div><small>Urgency</small><b>${b.emergency?'Urgent':'Standard'}</b></div><div><small>Request</small><b>${esc(b.requestSource==='VOICE'?'Voice':'Text')}</b></div></div></div>${assigned?`<div class="connected-state-line connected-worker-assigned"><span class="connected-step-label">ASSIGNED WORKER</span><h4>${esc(b.workerName)}</h4><p>${esc(b.workerVerification||'VERIFIED')} · ${b.distance!=null?`${esc(b.distance)} km · `:''}${esc(b.cooperative||'Cooperative')}</p></div>`:`<div class="connected-state-line"><b>${replacement?'Replacement search active':'Waiting for worker response'}</b><p>No worker appears assigned until that worker accepts.</p></div>`}`;
  }
  ...
  async function loadWorkerOffers(force=false){
    ...
    try{const offers=await request('/api/connected/worker/offers'),signature=JSON.stringify((offers||[]).map(o=>[o.offerId,o.offerStatus,o.status,o.bookingId,o.distance,o.total,o.scheduledAt]));if(!force&&signature===lastWorkerSignature)return;lastWorkerSignature=signature;root.innerHTML=offers.length?offers.map(offerCard).join(''):'<div class="connected-empty"><b>No job requests right now</b><br>Stay available. Suitable opportunities will appear here.</div>';wireOfferActions();}
    catch(err){root.innerHTML=`<div class="connected-error">${esc(friendlyError(err,'offer'))}</div>`;}
  }
  function offerCard(o){
    const accepted=o.offerStatus==='ACCEPTED';return `<article class="connected-offer" data-offer="${Number(o.offerId)}"><div class="connected-offer-head"><div><span class="badge ${accepted?'b-green':'b-orange'}">${accepted?'ACCEPTED':'NEW JOB REQUEST'}</span><h4>${esc(o.service)}</h4><p>${esc(o.zone||'Customer area')} ${o.distance!=null?`· ${esc(o.distance)} km away`:''}</p></div><div class="connected-earnings"><small>Expected Amount</small><b>₹${Number(o.total||0).toLocaleString('en-IN')}</b></div></div><div class="connected-meta"><div>Schedule<b>${o.scheduledAt?new Date(o.scheduledAt).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'}):'—'}</b></div><div>Cooperative<b>${esc(o.cooperative||'—')}</b></div><div>Booking ID<b>${esc(o.bookingCode||'—')}</b></div><div>Distance<b>${o.distance!=null?`${esc(o.distance)} km`:'—'}</b></div></div><div class="connected-state-line"><b>Customer Request</b><p>${esc(o.problem||o.voiceTranscript||'No additional description')}</p></div>${o.offerStatus==='PENDING'?`<div class="connected-actions connected-offer-actions"><button class="btn danger" data-reject-offer="${Number(o.offerId)}">Decline</button><button class="btn primary" data-accept-offer="${Number(o.offerId)}">Accept Job</button></div>`:'<div class="connected-success">Job accepted. This booking is assigned to you.</div>'}</article>`;
  }
  function wireOfferActions(){document.querySelectorAll('[data-accept-offer]').forEach(b=>b.onclick=()=>respondOffer(b,'ACCEPT'));document.querySelectorAll('[data-reject-offer]').forEach(b=>b.onclick=()=>openDecisionModal(b));}
  function openDecisionModal(button){const id=button.dataset.rejectOffer,root=document.getElementById('connectedModalRoot');root.innerHTML=`<div class="connected-modal-backdrop"><div class="connected-modal" role="dialog" aria-modal="true" aria-labelledby="declineTitle"><h3 id="declineTitle">Why are you declining this job?</h3><p>The reason is recorded. The same customer booking continues to the next eligible worker where possible.</p><div class="connected-choice-list">${['Schedule Conflict','Too Far','Unavailable','Not My Skill','Personal Reason','Other'].map((r,i)=>`<label><input type="radio" name="declineReason" value="${r}" ${i===0?'checked':''}><span>${r}</span></label>`).join('')}</div><div class="connected-actions"><button class="btn secondary" data-modal-cancel>Cancel</button><button class="btn danger" data-modal-confirm>Decline Job</button></div></div></div>`;root.querySelector('[data-modal-cancel]').onclick=closeDecisionModal;root.querySelector('[data-modal-confirm]').onclick=()=>{const reason=root.querySelector('input[name="declineReason"]:checked')?.value||'Other';closeDecisionModal();respondOffer(button,'REJECT',reason);};}
  ...
  async function respondOffer(button,action,reason=''){
    ...
    try{const result=await post(`/api/connected/worker/offers/${id}/respond`,{action,reason});lastWorkerSignature='';await loadWorkerOffers(true);workerMessage(action==='REJECT'?(result.nextWorker?'Job declined. The same booking is moving to the next eligible worker.':'Job declined. No penalty is applied in this demo workflow.'):'Job accepted. The customer has been notified.',action==='REJECT'?'warn':'success');signalSync('worker-offer');}
  ...
  function signalSync(source){window.dispatchEvent(new CustomEvent('sanpaid:connected-sync',{detail:{source,at:Date.now()}}));}
  function connectStream(){
    stopStream();if(!window.EventSource)return;stream=new EventSource('/api/connected/events',{withCredentials:true});
    stream.addEventListener('snapshot',event=>{try{const snapshot=JSON.parse(event.data);if(snapshot.role==='CUSTOMER'){const latest=snapshot.bookings?.[0];if(latest){activeBookingId=Number(latest.id);sessionSet(BOOKING_KEY,activeBookingId);refreshCustomerBooking();}}else if(snapshot.role==='WORKER')loadWorkerOffers();}catch{}});stream.onerror=()=>setLiveState('retry');stream.onopen=()=>setLiveState('online');
  }
  function stopStream(){if(stream){stream.close();stream=null;}}

  function installLandingButtons(){
    const match=document.getElementById('matching');if(match&&!document.getElementById('connectedDemoSection')){const sec=document.createElement('section');sec.id='connectedDemoSection';sec.className='section white';sec.innerHTML=`<div class="wrap"><div class="head"><span class="tag">Connected Role Proof</span><h2>One booking. Customer and worker views.</h2><p>Open the same backend workflow from the correct authenticated role.</p></div><div class="connected-grid"><div class="connected-card"><h3>Customer</h3><p>Request and track a verified local service.</p><button class="btn primary" data-open-connected="CUSTOMER">Open Customer</button></div><div class="connected-card"><h3>Worker A</h3><p>Receive an eligible opportunity and choose Accept or Decline.</p><button class="btn secondary" data-open-connected="WORKER_A">Open Worker A</button></div><div class="connected-card"><h3>Worker B</h3><p>Demonstrate same-booking replacement after decline.</p><button class="btn secondary" data-open-connected="WORKER_B">Open Worker B</button></div></div></div>`;match.insertAdjacentElement('afterend',sec);}
  }
