(() => {
  ...
  function close({clearIntent=true}={}){
    ...
    stopCustomerVoice(false);releaseCustomerPhotoUrl();releaseWorkerVoiceUrls();releaseWorkerPhotoUrls();
  ...
  function releaseCustomerPhotoUrl(){if(customerPhotoPreviewUrl){URL.revokeObjectURL(customerPhotoPreviewUrl);customerPhotoPreviewUrl='';}}
  function releaseWorkerPhotoUrls(){workerPhotoUrls.forEach(url=>URL.revokeObjectURL(url));workerPhotoUrls.clear();}
  ...
  function imageFromUrl(url){return new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>reject(new Error('IMAGE_DECODE'));image.src=url;});}
  function canvasJpeg(canvas,quality){return new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',quality));}
  async function prepareProblemPhoto(file){
    if(!file||file.size>12582912)throw new Error('PHOTO_TOO_LARGE');
    const sourceUrl=URL.createObjectURL(file);
    try{const image=await imageFromUrl(sourceUrl),naturalWidth=image.naturalWidth,naturalHeight=image.naturalHeight;if(!naturalWidth||!naturalHeight)throw new Error('PHOTO_INVALID');let limit=1600,quality=.82,blob=null,width=0,height=0;
      for(let attempt=0;attempt<5;attempt++){const scale=Math.min(1,limit/Math.max(naturalWidth,naturalHeight));width=Math.max(1,Math.round(naturalWidth*scale));height=Math.max(1,Math.round(naturalHeight*scale));const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const context=canvas.getContext('2d');context.drawImage(image,0,0,width,height);blob=await canvasJpeg(canvas,quality);if(blob&&blob.size<=1258291)break;limit=Math.round(limit*.78);quality=Math.max(.55,quality-.08);}
      if(!blob||blob.size>1258291)throw new Error('PHOTO_COMPRESS');return {blob,width,height};
    }finally{URL.revokeObjectURL(sourceUrl);}
  }
  async function selectProblemPhoto(event){
    const input=event.currentTarget,status=document.getElementById('cdPhotoStatus'),remove=document.getElementById('cdPhotoDelete'),file=input.files?.[0];if(!file)return;status.textContent='Preparing photo…';input.disabled=true;
    try{const prepared=await prepareProblemPhoto(file),data=await blobBase64(prepared.blob);releaseCustomerPhotoUrl();customerPhotoPreviewUrl=URL.createObjectURL(prepared.blob);problemPhotoMeta={data,mimeType:'image/jpeg',size:prepared.blob.size,width:prepared.width,height:prepared.height};document.getElementById('cdPhotoPreview').innerHTML=`<div class="connected-photo-preview"><img src="${esc(customerPhotoPreviewUrl)}" alt="Selected problem photo preview"><b>Problem photo ready · ${Math.ceil(prepared.blob.size/1024)} KB</b></div>`;remove.hidden=false;status.textContent='Photo attached. This exact photo will be shown to the offered worker.';}
    catch(error){deleteProblemPhoto();status.textContent=error?.message==='PHOTO_TOO_LARGE'?'Photo is too large. Choose an image under 12 MB.':'This photo could not be prepared. Choose a JPEG, PNG, WebP or another camera photo.';}
    finally{input.disabled=false;input.value='';}
  }
  function deleteProblemPhoto(){releaseCustomerPhotoUrl();problemPhotoMeta=null;const preview=document.getElementById('cdPhotoPreview'),remove=document.getElementById('cdPhotoDelete'),status=document.getElementById('cdPhotoStatus');if(preview)preview.innerHTML='';if(remove)remove.hidden=true;if(status)status.textContent='Optional: add one clear photo of the work or problem area.';}
  ...
  async function createConnectedBooking(event){
    ...
    if(problem.length<3&&!voiceMeta.message&&!problemPhotoMeta){error.innerHTML='<div class="connected-error">Describe the problem, attach a photo, or record a voice message before continuing.</div>';return;}
    ...
    try{
      const booking=await post('/api/connected/bookings',{service,zone:document.getElementById('cdZone').value.trim(),address:document.getElementById('cdAddress').value.trim(),problem,requestSource:isVoice?'VOICE':problemPhotoMeta?'PHOTO':'TEXT',requestLanguage:lang,voiceTranscript:isVoice?(voiceMeta.transcript||null):null,voiceMessage:isVoice?voiceMeta.message:null,problemPhoto:problemPhotoMeta,scheduledAt:scheduled.toISOString(),emergency:document.getElementById('cdEmergency').checked});
      activeBookingId=Number(booking.id);sessionSet(BOOKING_KEY,activeBookingId);progress.innerHTML='<div class="connected-success">Request created. Your attached details are available to the offered worker.</div>';renderCustomerState(booking,true);await refreshCustomerBooking();signalSync('customer-booking');releaseCustomerVoiceUrl();releaseCustomerPhotoUrl();voiceMeta={source:'TEXT',language:lang,transcript:'',message:null};problemPhotoMeta=null;document.getElementById('cdVoicePreview').innerHTML='';document.getElementById('cdVoiceDelete').hidden=true;document.getElementById('cdVoice').textContent='🎙 Record Voice Message';document.getElementById('cdVoiceStatus').textContent=isVoice?'Voice message sent securely with this booking.':'Optional: record up to 60 seconds. The worker will receive the actual audio.';document.getElementById('cdPhotoPreview').innerHTML='';document.getElementById('cdPhotoDelete').hidden=true;document.getElementById('cdPhotoStatus').textContent=booking.problemPhoto?.available?'Problem photo sent securely with this booking.':'Optional: add one clear photo of the work or problem area.';
  ...
  function offerCard(offer){
    const accepted=offer.offerStatus==='ACCEPTED',reasons=Array.isArray(offer.matching?.reasonCodes)?offer.matching.reasonCodes.slice(0,4):[],rank=Number(offer.rank||0),score=Number(offer.matching?.score||0),voice=offer.voiceMessage,photo=offer.problemPhoto;return `<article class="connected-offer" data-offer="${Number(offer.offerId)}"><div class="connected-offer-head"><div><span class="badge ${accepted?'b-green':'b-orange'}">${accepted?'ACCEPTED':'NEW JOB REQUEST'}</span><h4>${esc(offer.service)}</h4><p>${esc(offer.zone||'Customer area')} ${offer.distance!=null?`· DISTANCE ${esc(offer.distance)} km`:''}</p></div><div class="connected-earnings"><small>Service Amount</small><b>₹${Number(offer.total||0).toLocaleString('en-IN')}</b></div></div><div class="connected-meta"><div>Schedule<b>${offer.scheduledAt?new Date(offer.scheduledAt).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'}):'—'}</b></div><div>Cooperative<b>${esc(offer.cooperative||'—')}</b></div><div>Urgency<b>${offer.emergency?'Urgent':'Standard'}</b></div><div>Distance<b>${offer.distance!=null?`${esc(offer.distance)} km`:'—'}</b></div></div><div class="connected-state-line connected-offer-reason"><b>Why you received this offer</b><p>Eligibility gate passed${rank?` · Rank #${rank}`:''}${Number.isFinite(score)&&score>0?` · Match score ${score.toFixed(1)}`:''}.</p>${reasons.length?`<div class="connected-reason-tags">${reasons.map(r=>`<span>${esc(offerReason(r))}</span>`).join('')}</div>`:''}</div><div class="connected-state-line"><b>Customer Request</b><p>${esc(offer.problem||offer.voiceTranscript||'No additional description')}</p></div>${photo?.available?`<div class="connected-problem-photo" data-load-photo="${esc(photo.url)}" data-photo-booking="${Number(offer.bookingId)}"><span class="connected-step-label">CUSTOMER PROBLEM PHOTO</span><div class="connected-photo-loading" role="status">Loading customer photo…</div></div>`:''}${voice?.available?`<div class="connected-voice connected-voice-message" data-voice-booking="${Number(offer.bookingId)}"><span class="connected-step-label">CUSTOMER VOICE MESSAGE</span><p>Actual recording · ${voiceClock(voice.durationMs)}</p><button type="button" class="btn secondary" data-load-voice="${Number(offer.bookingId)}" data-voice-url="${esc(voice.url)}">▶ Load & Play Voice Message</button><div class="connected-voice-player" role="status" aria-live="polite"></div></div>`:''}${offer.offerStatus==='PENDING'?`<div class="connected-actions connected-offer-actions"><button type="button" class="btn danger" data-reject-offer="${Number(offer.offerId)}">Decline</button><button type="button" class="btn primary" data-accept-offer="${Number(offer.offerId)}">Accept Job</button></div>`:'<div class="connected-success">Job accepted. This booking is assigned to you.</div>'}</article>`;
  }
  function wireOfferActions(){document.querySelectorAll('[data-accept-offer]').forEach(button=>button.onclick=()=>respondOffer(button,'ACCEPT'));document.querySelectorAll('[data-reject-offer]').forEach(button=>button.onclick=()=>openDecisionModal(button));document.querySelectorAll('[data-load-voice]').forEach(button=>button.onclick=()=>loadWorkerVoice(button));document.querySelectorAll('[data-load-photo]').forEach(host=>loadWorkerPhoto(host));}
  ...
  async function loadWorkerPhoto(host){
    const loading=host.querySelector('.connected-photo-loading');
    try{const response=window.SanPaidApi?.raw?await window.SanPaidApi.raw(host.dataset.loadPhoto,{method:'GET'}):await fetch(host.dataset.loadPhoto,{credentials:'include',cache:'no-store'});if(!response.ok){const data=await response.json().catch(()=>({}));throw Object.assign(new Error(data.message||'Problem photo could not be loaded.'),{status:response.status});}const blob=await response.blob(),url=URL.createObjectURL(blob);workerPhotoUrls.add(url);host.innerHTML=`<span class="connected-step-label">CUSTOMER PROBLEM PHOTO</span><a href="${esc(url)}" target="_blank" rel="noopener" aria-label="Open customer problem photo full size"><img src="${esc(url)}" alt="Customer-provided problem photo"></a><small>Tap the photo to view full size.</small>`;}
    catch(error){if(loading)loading.textContent=friendlyError(error,'offer');}
  }
